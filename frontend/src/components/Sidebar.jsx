...
import { CalendarIcon, ChevronRightIcon, DollarSignIcon, FileTextIcon, LayoutGridIcon, LogIn, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, XIcon } from "lucide-react";
    ...
    <>
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto" >
        {navItems.map((item)=>{
          ...
          return (
            <Link key={item.name} to={item.href} className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
      ...
      <div className="p-3 border-t border-white/6">
          <button onClick={handleLogOut} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 cursor-pointer hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150 " >
